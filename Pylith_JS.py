...
class PyLith_JS():
    ...
    def FrictionTimeWeakeningFunction(self, mainDir,a, mu_s,mu_d, TimeWeakening):
        ...
        plt.figure(figN)
        #plt.rc('text',usetext=True)
        plt.plot(self.FaultX[:,0]/1e3, self.mu_f[:],'-b',linewidth=2,label='$\mu_s$')
        plt.plot(self.FaultX[:,0]/1e3, mu_s*np.ones(self.FaultX.shape[0]),'-k', linewidth=2, label='$\mu_d$')
        plt.legend(loc='upper right')
        plt.xlabel('X position along fault [km]')
        plt.ylabel('friction coefficient')
    ...
    def FindIndex(self,xcoord):
        ...
        xdata=np.array((self.FaultX[:,0]/1e3),dtype=int)
    ...
    def CreateInitialFaultStress(self, mainDir, mu, Coeff):
        ...
        figN=int(np.random.rand(1)*500)
        plt.figure(figN)
        plt.plot(self.FaultX[:,0]/1e3,np.abs(mu*self.FaultTraction2[:,0]),'-b',linewidth=2, label='Failure Criteria')
        plt.plot(self.FaultX[:,0]/1e3,np.abs(mu*self.FaultTraction2[:,0]*Coeff),'-k',linewidth=2, label='Initial shear stress')
    ...
    def GetIndexOfSSEOccurrence(self,mainDir,pos):
        ...
        figN=int(np.random.rand(1)*500)    
        OutputNameFig = mainDir+'/Figures/SSE_Locations.eps'
        ...
        for i in range(1,GPSXderivative.shape[0]):
            #if (GPSXderivative[i] >=0 and GPSXderivative[i-1] < 0):
            #    SlopeChange=np.append(SlopeChange,i-1)
            
            #if (GPSXderivative[i] <=0 and GPSXderivative[i-1] > 0):
            #    SlopeChangeTop=np.append(SlopeChangeTop,i)
                
            if (GPSXderivative[i]<0):
                SlopeChange=np.append(SlopeChange,i+1)
                SlopeChangeTop=np.append(SlopeChangeTop,i)
    ...
    def PlotDisplacementTimeSeries(self, OutputDir,FigName):
            ...
            for pos in range(0,self.Xtime.shape[1]):
                ...
                #plt.plot(self.year[:,pos],  self.XtimeNoTrend[:,pos]-self.Xtime[0,pos],'-',linewidth=1.5,label=self.nameGPS[pos])
    ...
    def PlotFaultSlipDuringSSE(self, mainDir,period_begin, period_end):
        ...
        print "Number of SSE events found with the specified periodicity=", self.SSEindex[ind[0,:],1].shape[0]
        
        if self.SSEindex[ind[0,:],1].shape[0] == 0:
            return
        
        for k in range(0,ind.shape[1]):
            plt.figure(1,[18,7])
            plt.subplot(1,2,1)
            plt.plot(self.FaultX[:,0]/1e3, self.disp1[ :, self.SSEindex[ind[0,k],1] ] - self.disp1[ :, self.SSEindex[ind[0,k],0] ], linewidth=2 )
            plt.xlabel('X position along fault [km]', fontsize=18)
            plt.ylabel('Fault displacement during SSE event [m]', fontsize=18)
            #plt.title('time='+str(self.FaultTime[ self.SSEindex[ind[0,k],1] ]))
            plt.title('Fault slip ', fontsize=18)
            plt.grid(True)
            plt.tick_params(labelsize=16)
            
            #diffTime=self.year[ind[0,:],1]-self.year[ind[0,:],0]
            y=self.disp1[ :, self.SSEindex[ind[0,k],1] ] - self.disp1[ :, self.SSEindex[ind[0,k],0] ]
            #print self.SSEduration.shape, self.SSEindex.shape
            #print "SSE duration=", self.SSEduration[ind[0,k]]
            y=y/self.SSEduration[ind[0,k]]
            
            ax=plt.subplot(1,2,2)
            ax.plot(self.FaultX[:,0]/1e3, y , linewidth=2 )
    ...
    def PlotGeometryWithFriction(self, mainDir):
        ...
        axarr[0].plot(self.FaultX[:,0]/1e3, self.FaultY[:,0]/1e3,'k', linewidth=3)
        axarr[0].plot(self.FaultX[:,0]/1e3, self.FaultY[:,0]/1e3-8,'--k', linewidth=1.5)
    ...
    def PlotFaultSlipDuringSSEAndGeometry(self, mainDir, period_begin, period_end):
        ...
        ax[0].plot(self.FaultX[:,0]/1e3, self.FaultY[:,0]/1e3,'k', linewidth=3)
        ax[0].plot(self.FaultX[:,0]/1e3, self.FaultY[:,0]/1e3-8,'--k', linewidth=1.5)
        ax[0].set_ylim([0,-80])
        ax[0].invert_yaxis()
        ...
        ax[0] = ax[0].twinx()
        lns2 = ax[0].plot(self.FaultX[:,0]/1e3, self.mu_f_s[:],'-b',linewidth=2,label='$\mu_s$')
        lns3 = ax[0].plot(self.FaultX[:,0]/1e3, self.mu_f_d[:],'-r',linewidth=2,label='$\mu_d$')
        ...
        for k in range(0,ind.shape[1]):
            ...
            ax[1].plot(self.FaultX[:,0]/1e3, self.disp1[ :, self.SSEindex[ind[0,k],1] ] - self.disp1[ :, self.SSEindex[ind[0,k],0] ], linewidth=2 )
            #plt.gca().invert_yaxis()
            #ax[1].invert_yaxis()
    ...
    def PlotMomentMagnitude(self, mainDir, shear_modulus):
        ...
        #Fault Area
        FaultArea=np.sum(np.sqrt(self.FaultX[:,0]**2 + self.FaultY[:,0]**2))
